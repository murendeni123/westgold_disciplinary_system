...
const ConsequenceManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Additional validation for suspensions
    if (formData.consequence_type === 'suspension' && (!formData.start_date || !formData.end_date)) {
      toast.warning('Start date and end date are required for suspensions');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <motion.button
          ...
          className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-red-500 to-pink-600 text-white rounded-xl font-medium shadow-lg hover:shadow-xl transition-all"
        >
        ...
        >
          <div className="bg-gradient-to-br from-yellow-500 to-amber-600 rounded-2xl p-6 shadow-lg text-white">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-3xl font-bold">{stats.verbal_warnings}</span>
            </div>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl p-6 shadow-lg text-white">
            <div className="flex items-center justify-between mb-4">
              <FileText size={24} />
              <span className="text-3xl font-bold">{stats.written_warnings}</span>
            </div>
          ...
          <div className="bg-gradient-to-br from-red-500 to-pink-600 rounded-2xl p-6 shadow-lg text-white">
            <div className="flex items-center justify-between mb-4">
              <Shield size={24} />
              <span className="text-3xl font-bold">{stats.suspensions}</span>
            </div>
      ...
      >
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-blue-900 mb-1">Administrator Permissions</h3>
            <p className="text-sm text-blue-700">
              As an administrator, you have full access to assign all consequence types including <strong>Suspensions</strong>. 
              You can also view and manage consequences assigned by teachers. The system includes automated evaluation to recommend consequences based on student behavior patterns.
            </p>
          </div>
        </div>
      </motion.div>
          ...
          >
            ...
            >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-white/80 text-sm mt-1">
                      Full administrator access - all consequence types available
                    </p>
                  </div>
                  <button
                    onClick={() => setShowModal(false)}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-140px)]">
                ...
                {/* Suspension Dates (only for suspensions) */}
                {formData.consequence_type === 'suspension' && (
                ...
                <div className="flex items-center space-x-3 pt-4 border-t border-gray-200">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    disabled={submitting}
                    className="flex-1 px-6 py-3 bg-gradient-to-r from-red-500 to-pink-600 text-white rounded-xl font-medium shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
                  >
                    {submitting ? (
                      <>
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900">{evaluation.student.total_incidents}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Total Points</p>
                      <p className="text-2xl font-bold text-gray-900">{evaluation.student.total_points}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Verbal Warnings</p>
                      <p className="text-2xl font-bold text-yellow-600">{evaluation.student.verbal_warnings}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Written Warnings</p>
                      <p className="text-2xl font-bold text-orange-600">{evaluation.student.written_warnings}</p>
                    </div>
                  </div>
                </div>
