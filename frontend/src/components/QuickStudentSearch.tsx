...
const QuickStudentSearch: React.FC = () => {
  ...
  const [dropdownStyle, setDropdownStyle] = useState<CSSProperties>({ position: 'fixed', visibility: 'hidden', zIndex: 9999 });
  ...
  const handleSelect = (student: any) => {
    ...
    setQuery('');
    setIsFocused(false);
    setDropdownStyle({ position: 'fixed', visibility: 'hidden', zIndex: 9999 });
  };
    ...
    <div ref={searchRef} className="relative w-full max-w-md">
      <div className="relative">
        ...
        <input
          ...
          type="text"
          value={query}
          onChange={(e) => { setQuery(e.target.value); updateDropdownPosition(); setIsFocused(true); }}
          onFocus={handleFocus}
          placeholder="Search students (Ctrl+K)..."
          className="w-full pl-12 pr-10 py-2.5 bg-white/80 backdrop-blur-sm border border-gray-200/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-300 transition-colors shadow-sm"
        />
        {query && (
          <button
      ...
      {showDropdown && ReactDOM.createPortal(
        <div
          style={dropdownStyle}
          className="bg-white border border-gray-200 rounded-2xl shadow-2xl max-h-80 overflow-y-auto"
        >
          {loading ? (
            ...
            <div className="py-2">
              {results.map((student) => (
                <button
                  key={student.id}
                  onMouseDown={(e) => { e.preventDefault(); handleSelect(student); }}
                  className="w-full px-4 py-3 text-left border-b border-gray-100 last:border-b-0 flex items-center space-x-3 hover:bg-blue-50 transition-colors"
                >
                      ...
                      <img
                        src={(() => {
                          const baseUrl = window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1'
                            ? 'http://192.168.18.160:5000' : 'http://localhost:5000';
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 truncate">ID: {student.student_id} • {student.class_name || 'No Class'}</p>
                  </div>
                </button>
              ))}
            </div>
