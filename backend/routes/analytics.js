    ...
    try {
        ...
        const thresholdStudents = await schemaAll(req, `
            SELECT s.id, s.student_id, s.first_name || ' ' || s.last_name as name,
                   c.class_name,
                   COALESCE(SUM(COALESCE(bi.points_deducted, bi.points, 0)), 0) as total_demerits,
            ...
            HAVING COALESCE(SUM(COALESCE(bi.points_deducted, bi.points, 0)), 0) >= 50
        ...
        const classesWithoutAttendance = await schemaAll(req, `
            ...
            AND c.id NOT IN (
                ...
                WHERE COALESCE(a.attendance_date, a.date) = $1 AND s.class_id IS NOT NULL
            )
        ...
        const pendingIncidents = await schemaAll(req, `
            SELECT bi.id, bi.description, COALESCE(bi.points_deducted, bi.points, 0) as points_deducted, bi.created_at,
    ...
    try {
        ...
        const repeatedAbsences = await schemaAll(req, `
            SELECT s.id, s.student_id, s.first_name || ' ' || s.last_name as name,
                   ...
                   MAX(COALESCE(a.attendance_date, a.date)) as last_absence
            FROM students s
            LEFT JOIN classes c ON s.class_id = c.id
            INNER JOIN attendance a ON s.id = a.student_id
            WHERE s.is_active = true
            AND a.status = 'absent'
            AND COALESCE(a.attendance_date, a.date) >= CURRENT_DATE - INTERVAL '30 days'
            GROUP BY s.id, s.student_id, s.first_name, s.last_name, c.class_name
            HAVING COUNT(a.id) >= 3
        ...
        const repeatDemerits = await schemaAll(req, `
            SELECT s.id, s.student_id, s.first_name || ' ' || s.last_name as name,
                   c.class_name,
                   COUNT(bi.id) as incident_count,
                   COALESCE(SUM(COALESCE(bi.points_deducted, bi.points, 0)), 0) as total_points,
...
router.get('/class-profile/:classId', authenticateToken, async (req, res) => {
    try {
        ...
        const students = await schemaAll(req, `
            SELECT s.id, s.student_id, s.first_name || ' ' || s.last_name as name,
                   COALESCE((SELECT SUM(COALESCE(points_deducted, points, 0)) FROM behaviour_incidents WHERE student_id = s.id), 0) as demerit_points,
